...
var app, colors, config, express, fs, git, jobs, path, requireAuth, runner, stylus, _;
...
requireAuth = express.basicAuth(function(user, pass) {
  if (!(config.concrete.auth != null)) {
    return true;
  }
  return config.concrete.auth.user === user && config.concrete.auth.password === pass;
});
...
app.get('/jobs', requireAuth, function(req, res) {
  return jobs.getAll(function(jobs) {
    return res.json(jobs);
  });
});

app.get('/job/:id', requireAuth, function(req, res) {
  return jobs.get(req.params.id, function(job) {
    return res.json(job);
  });
});

app.get('/job/:id/:attribute', requireAuth, function(req, res) {
  return jobs.get(req.params.id, function(job) {
...
app.get('/clear', requireAuth, function(req, res) {
  return jobs.clear(function() {
    return res.redirect("/jobs");
  });
});

app.get('/add', requireAuth, function(req, res) {
  return jobs.addJob(function() {
    return res.redirect("/jobs");
  });
});

app.get('/ping', requireAuth, function(req, res) {
