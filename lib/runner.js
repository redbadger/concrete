...
runTask = function(next) {
  var _this = this;
  jobs.updateLog(jobs.current, "Executing '" + this.config.runner + "'");
  return exec(this.config.runner, {
    maxBuffer: 1024 * 1024
      ...
      return updateLog(error, true, function() {
        return updateLog(stdout, true, function() {
          return updateLog(stderr, true, function() {
            return runFile(this.config.failure, next, false);
          });
        });
      });
    } else {
      return updateLog(stdout, true, function() {
        return runFile(this.config.success, next, true);
      });
    }
  });
};
